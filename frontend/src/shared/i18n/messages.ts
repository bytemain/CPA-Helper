...
const exactServerMessages: MessagePair[] = [
  ...
  ['账号 auth_index 身份冲突（列表与详情不一致），请刷新后重试', 'Account auth_index identity conflict (list and detail disagree). Refresh and try again.'],
  ['无法确定账号 auth_index，请刷新账号列表后重试', 'Cannot determine the account auth_index. Refresh the account list and try again.'],
  ['账号 account_id 身份冲突（列表与详情不一致），请刷新后重试', 'Account account_id identity conflict (list and detail disagree). Refresh and try again.'],
  ...
  ['账号身份冲突：Antigravity 列表与详情的 name/type/auth_index 不一致，已保留原快照', 'Account identity conflict: the Antigravity list and detail disagree on name/type/auth_index; the previous snapshot was preserved.'],
  ...
  ['核销主动重置额度失败：响应无效', 'Failed to redeem the reset credit: invalid response.'],
  ['核销主动重置额度失败：OpenAI 拒绝核销', 'Failed to redeem the reset credit: OpenAI rejected the redemption.'],
  ['核销主动重置额度失败：响应状态未知', 'Failed to redeem the reset credit: unknown response status.'],
