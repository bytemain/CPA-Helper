...
func TestFinalizeKeeperQuotaWindowUsageProjection(t *testing.T) {
	windowSeconds := 3600
	...
	cases := []struct {
		...
		wantElapsed    int
		wantPercent    float64
		wantProjection float64
	}{
		{"halfway", start.Add(30 * time.Minute), false, 10, 1800, 50, 20},
		{"just started", start.Add(2 * time.Minute), false, 3, 120, 3.33, 90},
		{"no elapsed", start, false, 5, 0, 0, 0},
		{"stale projects to own total", start.Add(2 * time.Duration(windowSeconds) * time.Second), true, 42, 3600, 100, 42},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			usage := &keeperQuotaWindowUsage{
				WindowStart:      start,
				WindowEnd:        start.Add(time.Duration(windowSeconds) * time.Second),
				WindowSeconds:    windowSeconds,
				EstimatedCostUSD: tc.cost,
				Stale:            tc.stale,
			}
			finalizeKeeperQuotaWindowUsage(usage, tc.now)
			if usage.ElapsedSeconds != tc.wantElapsed {
				t.Fatalf("elapsed = %d, want %d", usage.ElapsedSeconds, tc.wantElapsed)
			}
			if usage.WindowElapsedPercent != tc.wantPercent {
				t.Fatalf("elapsed percent = %v, want %v", usage.WindowElapsedPercent, tc.wantPercent)
			}
			if usage.ProjectedCostUSD != tc.wantProjection {
				t.Fatalf("projected cost = %v, want %v", usage.ProjectedCostUSD, tc.wantProjection)
			}
		})
	}
}

func TestComputeKeeperQuotaWindowUsagesFillsProjection(t *testing.T) {
	t.Setenv("CPA_HELPER_DATA_DIR", t.TempDir())
	app, err := New()
	if err != nil {
		t.Fatalf("New() failed: %v", err)
	...
	now := time.Date(2026, 8, 21, 12, 30, 0, 0, appTimeLocation)
	resetAt := now.Add(30 * time.Minute)
	accounts := []keeperAccount{
		{
			Name:                 "proj.json",
			Email:                stringPtr("proj@example.com"),
			AccountType:          stringPtr("plus"),
			PrimaryResetAt:       timePtrValue(resetAt),
	...
	insertKeeperWindowUsageRecord(t, app, keeperWindowUsageSeed{
		Dedupe:       "proj-record",
		Timestamp:    now.Add(-10 * time.Minute),
		Source:       "proj@example.com",
		InputTokens:  100,
		OutputTokens: 20,
		RawJSON:      `{"source":"proj@example.com"}`,
	})
	...
	usage := usages["proj.json"].Primary
	if usage == nil {
		t.Fatal("primary usage missing")
	}
	if usage.ElapsedSeconds != 1800 {
		t.Fatalf("elapsed = %d, want 1800", usage.ElapsedSeconds)
	}
	if usage.WindowElapsedPercent != 50 {
		t.Fatalf("elapsed percent = %v, want 50", usage.WindowElapsedPercent)
	}
	if want := mathRound(usage.EstimatedCostUSD*2, 8); usage.ProjectedCostUSD != want {
		t.Fatalf("projected = %v, want %v (2x observed)", usage.ProjectedCostUSD, want)
	}
	resp := keeperQuotaWindowUsageResponseFrom(usage)
	if resp.WindowElapsedPercent != usage.WindowElapsedPercent || resp.ProjectedCostUSD != usage.ProjectedCostUSD {
		t.Fatal("response mapping drops projection fields")
	}
}
